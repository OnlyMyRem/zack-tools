...
- 版式：与时间戳页一样不写自己的页头也不留页脚（面包屑末项「Cron 表达式」即本页可见的 `<h1>`），
  顶部一块面板收着表达式、推算时区、格式与它的解释、状态与人话总结；其下**常用预设在左栏 sticky，
  未来触发时间与逐字段解析在右栏**，一屏内就能同时对着表达式、预设和触发时刻，不必再上下滚。
  字段生成器与语法小抄退到下方整行。三种格式的解释叠在同一个网格单元里，占位恒为最长那条，
  切换方言只换可见性，下面的内容不再被顶来顶去。
- 三种方言：**Quartz**（6/7 段，周从 1 起，`?`/`L`/`W`/`#`/年）、**Spring**（6 段，周 0-7）、
  **Linux crontab**（5 段，无秒、无 `?`、日与周同时指定取并集）。
- 切换方言时搬运原始字段做互转，并明确提示有损转换（丢弃年段、`L/W/#` 在 Linux 不认、
  并集↔交集语义变化），不静默改用户的约束。
- 逐字段中文解析 + 一句话人话总结；段数不符、`?` 违规、值越界都给可操作的报错。
- 未来触发时间预览（5/10/20/50 次），按选定时区推算，含夏令时空隙处理与 100 年收敛上限。
